...
export function initRenderer(gl, program, cameraPosition, cameraTarget) {
    meshProgramInfo = {
        program,
        attribLocations: {
    ...
    projectionMatrix = m4.perspective(Math.PI / 3, aspect, 0.1, 1000);
}
...
export function renderScene(gl, meshes, time, objOffset = [0, 0, 0]) {
    ...
    const worldMatrix = m4.translate(m4.yRotation(time * 0.5), ...objOffset);
