...
function update() {
    ...
    // 2. spider.update(controllerNode)에서 solveIK로 각도 갱신 (worldMatrix 갱신 X)
    spider.update(controllerNode);

    // 3. spider 트리의 localMatrix만 갱신 (worldMatrix는 sceneRootNode에서 다시 갱신)
    spider.root.traverse(node => node.updateLocalMatrix());

    // 4. 마지막에 sceneRootNode.computeWorld()로 전체 트리 worldMatrix 갱신
    sceneRootNode.computeWorld();
}
